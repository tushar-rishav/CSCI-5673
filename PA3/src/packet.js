...
const writeInMillis = (buffer, offset, ts, addDelta) => {
	...
	// seconds
	buffer[offset + 0] = (seconds & 0xff000000) >> 24; // Ask Akhil: Why right shift 24? 
...
class NTPPacket {
	...
	bufferize(packet) {
		const buffer = Buffer.alloc(48).fill(0x00); // 48 bytes size NTP packet
		...
		buffer.writeUInt32BE(packet.rootDispersion, 8); // skip first 8 bytes
		buffer.writeUInt32BE(packet.referenceId, 12); // skip first 12 bytes
...
module.exports = { NTPPacket, MODES };
